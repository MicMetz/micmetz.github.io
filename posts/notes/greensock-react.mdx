...
<IntroParagraph >
		...
		Using the [GreenSock](https://greensock.com/) web animation library (also known as gsap) in vanilla JavaScript isn't too complex. But when we want it to play nicely with a frontend framework like
		React, we run into some finicky issues.
