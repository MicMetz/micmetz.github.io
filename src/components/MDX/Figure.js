...
export default function Figure( {
  ...
  useEffect( () => {
    ...
    const imageDims = mediaDimensions[ imagePath ] || mediaDimensions[ imagePath + '.jpg' ];
    if ( imageDims ) {
      setImage( imageDims );
    }

    const videoPath = dir + videoSrc;
    const videoDims = mediaDimensions[ videoPath ] || mediaDimensions[ videoPath + '.mp4' ];
    if ( videoDims ) {
      setVideo( videoDims );
    }
  ...
  let frame = {};
  if ( manualWidth ) {
    frame = { width: manualWidth };
  } else if ( manualHeight ) {
    frame = { maxHeight: manualHeight };
  } else if ( show === 'image' ) {
    frame = autoSize( image, sectionWidth );
  } else if ( show === 'video' ) {
    frame = autoSize( video, sectionWidth );
  }
  ...
  let caption = '';
  if ( show === 'image' ) {
    caption = imageCaption || neutralCaption;
  }
  if ( show === 'video' ) {
    caption = videoCaption || neutralCaption;
  }
  ...
  return (
    <FigureContainer id = {id} data-show = {show} >
      {imageSrc && videoSrc && (
        <FigureControls style = {controls} >
          <ClickableObject
            icon = 'far fa-image'
            text = 'Still'
            active = {show === 'image'}
...
const FigureControls = styled.div`
  max-width: 100%;
  text-align: right;
